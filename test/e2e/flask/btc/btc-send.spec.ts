...
describe('BTC Account - Send', function () {
  const recipientAddress = 'bc1qsqvczpxkgvp3lw230p7jffuuqnw9pp4j5tawmf';
  const pages = {
    home: BitcoinHomepage,
    send: BitcoinSendPage,
    review: BitcoinReviewTxPage,
  };
  ...
  it('can complete the send flow', async function () {
    ...
    await withBtcAccountSnap(async (driver) => {
      const homePage = new pages.home(driver);
      await homePage.check_pageIsLoaded();
      await homePage.check_isExpectedBitcoinBalanceDisplayed(DEFAULT_BTC_BALANCE);
      await homePage.startSendFlow();

      const bitcoinSendPage = new pages.send(driver);
      ...
      const bitcoinReviewTxPage = new pages.review(driver);
      ...
      await bitcoinReviewTxPage.check_feeRateIsDisplayed(Math.floor(DEFAULT_BTC_FEE_RATE).toString());
  ...
  it('can send the max amount', async function () {
    ...
    await withBtcAccountSnap(async (driver) => {
      ...
      const bitcoinReviewTxPage = new pages.review(driver);
      await bitcoinReviewTxPage.check_pageIsLoaded();
      await bitcoinReviewTxPage.check_sendAmountIsDisplayed((DEFAULT_BTC_BALANCE - expectedFee).toString());
      await bitcoinReviewTxPage.check_totalAmountIsDisplayed(DEFAULT_BTC_BALANCE.toString());
      await bitcoinReviewTxPage.clickSendButton();
